...
{
public:
    SwerveModule(std::shared_ptr<can::TalonSRX> driveController, std::shared_ptr<
                 can::TalonSRX> rotateController);
    virtual
    ~SwerveModule();
    ...
    GetAngle() const;
    void
    SetAngle(Rotation2D angle, bool doOptimization);
    void
    Set(double speed, Rotation2D angle, bool doOptimization);
    void
    Stop();
    double
    GetVelocity() const;
