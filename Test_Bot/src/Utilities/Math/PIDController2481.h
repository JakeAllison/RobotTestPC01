...
{
public:
    PIDController2481(double p, double i, double d, PIDSource* source, PIDOutput* output, double period = 0.05);
    PIDController2481(double p, double i, double d, double f, PIDSource* source, PIDOutput* output, double period = 0.05);
    PIDController2481(double p, double i, double d, PIDSource& source, PIDOutput& output, double period = 0.05);
    PIDController2481(double p, double i, double d, double f, PIDSource& source, PIDOutput& output, double period = 0.05);
    ...
    PIDController2481&
    operator=(const PIDController2481) = delete;

    virtual double
    Get() const;
    virtual void
    SetContinuous(bool continuous = true);
    virtual void
    SetInputRange(double minimumInput, double maximumInput);
    virtual void
    SetOutputRange(double minimumOutput, double maximumOutput);
    void
    SetPID(double p, double i, double d) override;
    virtual void
    ...
    void
    SetSetpoint(double setpoint) override;
    double
    GetSetpoint() const override;
    double
    GetDeltaSetpoint() const;
    ...
    virtual PIDSourceType
    GetPIDSourceType() const;
    ...
    virtual void
    SetPercentTolerance(double percentValue);
    ...
    void
    InitSendable(SendableBuilder& builder) override;
...
protected:
    ...
    double
    GetContinuousError(double error) const;
...
private:
    ...
    enum {
        kAbsoluteTolerance, kPercentTolerance, kNoTolerance
