...
public:
    ...
    Rotation2D
    rotateBy(const Rotation2D &other) const;
    Rotation2D
    inverse() const;
    ...
    virtual Rotation2D
    interpolate(const Rotation2D &other, double x) const;
    Rotation2D
    extrapolate(const Rotation2D& other, double x) const;
private:
    double m_cos;
    double m_sin;
